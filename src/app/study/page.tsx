...
export default function StudyPage() {
  ...
  const startLevel = (level: StudyLevel) => {
    setSelected(level);
    setPhase('content');
  };
  ...
  const handleAnswer = (idx: number) => {
    ...
    const question = selected!.quiz[quizIndex];
  ...
  const finishLevel = () => {
    const correctCount = [...answers, chosenIndex === selected!.quiz[quizIndex].correctIndex ? true : false].filter(Boolean).length;
    const ratio = correctCount / selected!.quiz.length;
      ...
      <AnimatePresence mode="wait">
        {phase === 'list' && (
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <h1 className="heading-serif text-2xl mb-1" style={{ color: '#F59E0B' }}>
              学習ステージ
            </h1>
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: done ? '0 0 10px rgba(16,185,129,0.4)' : 'none',
                        }}
                      >
                        {done ? <CheckCircle size={20} color="#fff" /> : locked ? <Lock size={18} color="#475569" /> : <BookOpen size={20} color="#0F172A" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-bold text-sm truncate" style={{ color: done ? '#10B981' : '#F8FAFC' }}>
                            {level.title}
                          </span>
                          <span className="subject-badge flex-shrink-0">{level.subject}</span>
                        </div>
                        <p className="text-xs text-ink-muted truncate">{level.description}</p>
                      </div>
                      <div className="flex flex-col items-end flex-shrink-0">
                        ...
                        {!locked && !done && <ChevronRight size={16} className="text-ink-subtle mt-1" />}
                      </div>
                    </button>
                  </motion.div>
                );
        ...
        {phase === 'content' && selected && (
          <motion.div key="content" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <button onClick={() => setPhase('list')} className="text-ink-muted text-sm mb-4 flex items-center gap-1">
              ← 戻る
            </button>
            <div className="glass-card p-5 mb-4">
              ...
              <h2 className="heading-serif text-xl mb-3" style={{ color: '#F59E0B' }}>{selected.title}</h2>
              <div
                className="text-sm leading-relaxed text-ink prose-custom"
                style={{ whiteSpace: 'pre-wrap', color: '#E2E8F0' }}
                dangerouslySetInnerHTML={{ __html: formatContent(selected.content) }}
              />
            </div>
            <button className="btn-gold w-full" onClick={startQuiz}>
              理解できた！クイズに挑戦 →
            </button>
          </motion.div>
        )}
        ...
        {phase === 'quiz' && selected && currentQuestion && (
          <motion.div key={`quiz-${quizIndex}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div className="flex gap-1">
                {selected.quiz.map((_, i) => (
                  <div
                    ...
                    style={{
                      background: i < answers.length ? (answers[i] ? '#10B981' : '#FB7185') : i === quizIndex ? '#F59E0B' : 'rgba(255,255,255,0.2)',
                    }}
                  />
                ))}
              </div>
            ...
            <div className="flex flex-col gap-3 mb-4">
              {currentQuestion.options.map((opt, i) => {
                ...
                return (
                  ...
                  >
                    <span className="font-bold mr-2" style={{ color: '#F59E0B' }}>{String.fromCharCode(65 + i)}.</span>
                    {opt}
                  </button>
                );
              })}
            ...
            <AnimatePresence>
              {showExplanation && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="glass-card p-4 mb-4" style={{ border: '1px solid rgba(245,158,11,0.3)' }}>
        ...
        {phase === 'result' && selected && (
          <motion.div key="result" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center pt-8">
            ...
            <p className="text-ink-muted mb-6">{answers.filter(Boolean).length} / {selected.quiz.length} 問正解</p>
            <div className="glass-card p-4 mb-6 flex items-center justify-center gap-2" style={{ border: '1px solid rgba(245,158,11,0.3)' }}>
              ...
              <span className="font-bold" style={{ color: '#F59E0B' }}>XP獲得！</span>
            </div>
            <button className="btn-gold w-full" onClick={() => setPhase('list')}>
              ステージ一覧に戻る
            </button>
          </motion.div>
        )}
...
function formatContent(md: string): string {
  return md
    .replace(/^## (.+)$/gm, '<h2 style="font-family:var(--font-serif);color:#F59E0B;font-size:1.1rem;font-weight:700;margin:1rem 0 0.5rem">$1</h2>')
    .replace(/^### (.+)$/gm, '<h3 style="color:#FCD34D;font-size:0.9rem;font-weight:700;margin:0.75rem 0 0.25rem">$1</h3>')
    .replace(/\*\*(.+?)\*\*/g, '<strong style="color:#FCD34D">$1</strong>')
    .replace(/^> (.+)$/gm, '<blockquote style="border-left:3px solid #F59E0B;padding-left:1rem;margin:0.75rem 0;color:#94A3B8;font-size:0.85rem">$1</blockquote>')
    .replace(/\n/g, '<br/>');
}
